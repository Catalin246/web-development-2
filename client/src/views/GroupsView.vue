...
function formatMessageTime(timestamp) {
  ...
  if (diff < oneDay) return timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
...
const calculatedChats = computed(() =>
  chats.value
    .filter(chat => chat.type === props.chatType)
    .map(chat => {
      const unreadCount = chat.messages.filter(m => !m.read).length
      return {
        ...chat,
        unread: unreadCount,
...
<template>
  <!-- Chat List -->
  <div v-if="!selectedChat || windowWidth >= 768" class="w-full md:w-[30%] overflow-y-auto shadow-right">
    <div v-for="chat in calculatedChats" :key="chat.name" @click="openChat(chat)">
      <ChatItem
        ...
        :time="chat.messages.at(-1)?.formattedTime || ''"
      />
    </div>
  </div>
